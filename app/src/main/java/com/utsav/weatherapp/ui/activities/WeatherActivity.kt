package com.utsav.weatherapp.ui.activities
...
import com.utsav.weatherapp.databinding.ActivityWeatherBinding
import com.utsav.weatherapp.ui.adapters.CustomAdapterSearchedCityTemperature
import com.utsav.weatherapp.ui.adapters.WeeklyWeatherAdapter
import com.utsav.weatherapp.ui.viewmodel.WeatherViewModel
...
class WeatherActivity : AppCompatActivity(), KodeinAware {
    ...
    private fun initializeRecyclerView() {
        ...
        val mLayoutManagerWeekly = LinearLayoutManager(
            this,
            LinearLayoutManager.VERTICAL,
            false
        )
        dataBind.recyclerViewWeekly.apply {
            layoutManager = mLayoutManagerWeekly
            itemAnimator = DefaultItemAnimator()
    ...
    private fun observeAPICall() {
        ...
        viewModel.weatherWeeklyLiveData.observe(this, EventObserver { state ->
            when (state) {
                is State.Loading -> {
                }
                is State.Success -> {
                    if (state.data.list.isEmpty()) {
                        dataBind.recyclerViewWeekly.hide()
                    } else {
                        dataBind.recyclerViewWeekly.show()
                        weeklyWeatherAdapter.setData(state.data.list)
                    }
                }
                is State.Error -> {
                    Log.e("ERROR", "observeAPICall: " + state.message)
                    showToast(state.message)
                }
            }
        })
